...
# Disabilita update check ultralytics (evita chiamate a GitHub)
...
ANGLE_THRESHOLD = 20    # gradi per sterzo
Y_THRESHOLD     = 160    # pixel: zona neutra verticale (± dal centro)

DURATA_COMANDO  = 0.5   # secondi: durata per AVANTI/INDIETRO
DURATA_COMANDO_STERZO = 0.2   # secondi: durata specifica per le rotazioni
DELAY_COMANDO   = 0.5   # secondi: pausa (fermo) tra un comando e il successivo

#DURATA_COMANDO  = 0.5   # secondi: durata per AVANTI/INDIETRO
#DURATA_COMANDO_STERZO = 0.5   # secondi: durata specifica per le rotazioni
#DELAY_COMANDO   = 3.0   # secondi: pausa (fermo) tra un comando e il successivo
# ─────────────────────────────────────────────────────────────────────────────
...
UPPER_ORANGE_YEL    = np.array([45,  255, 255])   # esteso a H=45 per giallo caldo
FIRE_KERNEL         = np.ones((3, 3), np.uint8)
FIRE_HISTORY_LEN    = 5    # frame nella finestra temporale di movimento
FIRE_MOV_THRESH     = 3000 # pixel che cambiano per considerare movimento
FIRE_PERSIST_MIN    = 3    # frame consecutivi per confermare il fuoco
# ─────────────────────────────────────────────────────────────────────────────
...
if _ULTRALYTICS_OK:
    if os.path.exists(_MODEL_PATH):
        ...
        print(f"[INFO] fire_model.pt non trovato — esegui scarica_modello.py con connessione internet")
        print(f"       Percorso atteso: {_MODEL_PATH}")

# ── THREAD LETTURA FRAME (con riavvio automatico ffmpeg) ─────────────────────
frame_queue    = queue.Queue(maxsize=2)
STALL_TIMEOUT  = 6.0   # secondi senza frame → riavvia ffmpeg

def _frame_reader():
    global ffmpeg_proc
    last_frame_t = time.time()
    while True:
        # Legge con timeout simulato: controlla ogni 0.5 s se ffmpeg è bloccato
        proc = ffmpeg_proc
        try:
        ...
        if len(raw) == FRAME_SIZE:
            ...
            # Nessun frame completo — verifica se ffmpeg è bloccato
...
# ── STATO TEMPORIZZAZIONE ────────────────────────────────────────────────────
phase          = "IDLE"
phase_start    = 0.0
active_cmd     = b'S'
active_dir     = "FERMO"
active_duration = DURATA_COMANDO # Variabile dinamica per la durata corrente

# ── PAUSA ARUCO ──────────────────────────────────────────────────────────────
pausa_aruco = False   # toggle con K: blocca i comandi ArUco, drone fermo

# ── MODALITÀ ─────────────────────────────────────────────────────────────────
mode               = "COMANDO"  # "COMANDO" | "FUOCO"  — toggle con P
fire_engine        = "YOLO" if YOLO_AVAILABLE else "HSV"  # toggle con F
    ...
    while True:
        try:
            ...
            if last_frame is not None:
                # Riusa l'ultimo frame valido: nessun lampeggio
                frame = last_frame.copy()
            else:
                waiting = np.zeros((H, W, 3), dtype=np.uint8)
                cv2.putText(waiting, "In attesa dello stream RTMP...",
                            (W//2 - 220, H//2), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 200, 255), 2)
                cv2.putText(waiting, "rtmp://172.20.10.2/live/drone",
                            (W//2 - 190, H//2 + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (180, 180, 180), 1)
        ...
        if mode == "FUOCO":
            ...
            if fire_engine == "YOLO" and YOLO_AVAILABLE:
                ...
                for box in results.boxes:
                    ...
                    cv2.putText(frame, f"{label} {conf:.0%}",
                                (x1, max(y1 - 10, 10)),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, colore_box, 2)

                # Overlay maschera HSV semitrasparente come ausilio visivo
            ...
            # ══ RAMO HSV puro ════════════════════════════════════════════════
                ...
                for contour in contours:
                    ...
                    if circularity < 0.7 and solidity < 0.90 and movement:
                        ...
                        cv2.putText(frame, "Possibile fuoco" if fire_persist_count < FIRE_PERSIST_MIN else "FUOCO",
                                    (x, max(y - 10, 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.85, colore_box, 2)
                        fire_found = True

            # ── Persistenza temporale (comune a entrambi i rami) ─────────────
            if fire_found:
            ...
            cv2.putText(frame, f"[ RILEVAMENTO FUOCO: {motore} ]  [F]=switch  [P]=Comando",
                        (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 140, 255), 2)
            cv2.putText(frame, "IIS Enzo Ferrari - ROMA", (15, 75),
            ...
            cv2.putText(frame, stato_fuoco, (15, 115),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, colore_stato, 2)
            cv2.putText(frame, f"Confidenza: {min(fire_persist_count, FIRE_PERSIST_MIN)}/{FIRE_PERSIST_MIN}",
                        (15, 145), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
            cv2.putText(frame, "[ESC]=esci  [p]=switch modalita'  [f]=YOLO/HSV",
                        (15, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 0, 0), 1)

        # ══════════════════════════════════════════════════════════════════════
        # MODALITÀ: STAZIONE DI COMANDO
        # ══════════════════════════════════════════════════════════════════════
        else:
            ...
            if ids is not None and len(ids) > 0:
                ...
                cv2.putText(frame, f"A:{current_angle:.1f}",
                            (cx - 40, cy - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 1)
                cv2.putText(frame, f"Y offset: {cy - frame_center[1]}px",
                            (cx - 40, cy - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 1)

            control_dir = "FERMO" if pausa_aruco else detected_dir

            # ── Macchina a stati temporizzata ────────────────────────────────
            if phase == "IDLE":
                ...
                if elapsed >= active_duration:
                    ...
                    phase       = "PAUSA"
                    phase_start = now
                cv2.putText(frame, f"CMD: {remaining:.1f}s",
                            (w - 160, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                ...
                if elapsed >= DELAY_COMANDO:
                    phase = "IDLE"
                cv2.putText(frame, f"PAUSA: {remaining:.1f}s",
                            (w - 170, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 100, 255), 2)
            ...
            cv2.putText(frame, "ZONA NEUTRA (sterzo/fermo)", (10, zone_top - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 150, 0), 1)
            cv2.putText(frame, "v INDIETRO", (w//2 - 50, zone_top - 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
            cv2.putText(frame, "^ AVANTI",   (w//2 - 60, zone_bottom + 25),
            ...
            if pausa_aruco:
                ...
                cv2.putText(frame, "[ PAUSA - ArUco disabilitato ]  [K]=riprendi",
                            (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 0), 2)

            hud_y = 65 if pausa_aruco else 30
            cv2.putText(frame, "IIS Enzo Ferrari - ROMA",              (15, hud_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 1,   (255,0,0), 2)
            cv2.putText(frame, f"Rilevato: {detected_dir}",            (15, hud_y + 35),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)
            cv2.putText(frame, f"Cmd ESP32: {CMD_LABELS[active_cmd]}", (15, hud_y + 70),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,200,255), 2)
            cv2.putText(frame, f"Fase: {phase}",                       (15, hud_y + 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,200,0), 1)
            if reference_angle is not None:
                cv2.putText(frame, f"Ref angolo: {reference_angle:.1f}",
                            (15, hud_y + 125),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,255), 1)
            cv2.putText(frame, "[ESC]=esci  [p]=Rilev.Fuoco  [c]=calibra  [r]=reset  [k]=pausa",
                        (15, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255,0,0), 1)
